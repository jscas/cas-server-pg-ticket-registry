...
Registry.prototype.trackServiceLogin = function tsl(st, tgt) {
  const TGT = this.db.models.TicketGrantingTicket;

  function* generator() {
    const serviceIds = tgt.extra || [];
    if (serviceIds.indexOf(st.serviceId) == -1) {
      serviceIds.push(st.serviceId);
    }
    tgt.extra = serviceIds;
    log.debug('tgt serviceIds: %j', tgt.extra);

    try {
      log.debug('registering service "%s" with tgt: %s', st.serviceId, tgt.tid);
      tgt.markDirty();
      yield TGT.query().patch({extra: serviceIds}).where('tid', tgt.tid);
    } catch (e) {
      log.debug('message: %s', e.message);

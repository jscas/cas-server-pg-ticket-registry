...
Registry.prototype.genTGT = function genTGT(loginTicketId, userId, expires) {
  ...
  function* generator() {
    ...
    try {
      tgt = yield TGT.query().insert(tgt).returning('*');
    } catch (e) {
...
Registry.prototype.genST = function genST(ticketGrantingTicketId, expires) {
  ...
  function* generator() {
    ...
    try {
      st = yield ST.query().insert(st).returning('*');
    } catch (e) {
...
function invalidateTicket(type, id) {
  ...
  function* generator() {
    ...
    try {
      ...
      ticket = yield DB.query().where('tid', id).first();
...
Registry.prototype.getSTbyTGT = function getSTbyTGT(id) {
  const ST = this.db.models.ServiceTicket;

  function* generator() {
    log.debug('retrieving st by tgt: %s', id);
    try {
      const tgt = yield this.getTGT(id);
      const st = yield ST.query().where('tgt_id', tgt.id).first();
      log.debug('retrieved st: %s', st.tid);
      return st;
    } catch (e) {
      log.debug('message: %s', e.message);
      log.debug('detail: %s', e.detail);
...
Registry.prototype.trackServiceLogin = function tsl(st, tgt, serviceUrl) {
  ...
  function* generator() {
    ...
    try {
      ...
      // not patching because https://github.com/Vincit/objection.js/issues/114
      // yield TGT.query().patch({extra: logoutServices}).where('tid', tgt.tid);
      yield TGT.query().update(tgt).where('tid', tgt.tid);
...
Registry.prototype.servicesLogForTGT = function slft(tid) {
  const TGT = this.db.models.TicketGrantingTicket;
  ...
  function* generator() {
    ...
    let services = [];
    try {
      const ticket = yield TGT.query().where('tid', tid).first();
      services = ticket.extra;
    } catch (e) {
      log.debug('message: %s', e.message);
      log.debug('detail: %s', e.detail);
    }
    log.debug('services for tgt: (%s, %j)', tid, services);
    return services;
  }
